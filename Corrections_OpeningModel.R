...
for (i in 1:Iterations){
  ...
  ProbabilityMatrix [,2] <- 1-(othreshold*ProbabilityMatrix[,2])
        ...
        ## Allow for manipulation of daily percentage coeficients 
  for (d in 2:2){
    ProbabilityMatrix[2,2] <- ProbabilityMatrix[2,2]*0
  }
  for (d in 3:4){
    ProbabilityMatrix[d,2] <- ProbabilityMatrix[d,2]*0
  }
  for (d in 5:6){
    ProbabilityMatrix[d,2] <- ProbabilityMatrix[d,2]*0
  }
  ...
  for (d in 12:12){
    ProbabilityMatrix[12,2] <- ProbabilityMatrix[12,2]*0 
  }
  ...
  if (is.na(OpeningDay) == TRUE) {
    OpeningDay <- NA
    ## Calculate probability of opening on subsequent days
  } else if (OpeningDay == 12){
    Latency[12] <- ProbabilityMatrix[12,1]/ProbabilityMatrix[12,2] * 40
    Opening[12] <- TRUE
  } else if (OpeningDay == 11){
    Latency[11] <- ProbabilityMatrix[11,1]/ProbabilityMatrix[11,2] * 40
    ...
    for (w in (OpeningDay+1):11){
      ...
      if (ProbabilityMatrix[w,1] < ProbabilityMatrix[w,2]){
        ...
        Opening[w] <- FALSE
        ProbabilityMatrix[w+1,2] <- ProbabilityMatrix[w+1,2]*1.1
        Latency[w] <- 40
      }
    }
